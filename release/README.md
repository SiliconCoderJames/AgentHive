...
├─ wix/
│  ├─ MiderHive.wxs           ← MSI 定义（per-user 安装、开始菜单/桌面快捷方式、升级与卸载策略）
│  └─ files.generated.wxs     ← 由 scripts/gen-wix-files.ps1 从暂存目录生成（派生文件，不入库）
├─ MiderHive-<版本>-x64.msi   ← 安装包（产物，不入库）
├─ MiderHive-<版本>-win64-portable.zip ← 便携版（产物，不入库）
...
1. `cmake` 配置 + 编译（版本号经 `-DMIDERHIVE_VERSION` 注入）
...
6. 打包便携版 ZIP（内含一层 `MiderHive-<版本>/` 目录，解压不散落）
7. 生成 `SHA256SUMS.txt`
...
安装包属性三处共用。发布时用 `-DMIDERHIVE_VERSION`（或 CI 传标签名）覆盖。

> **注意**：PowerShell 5.1 会把**未加引号**的 `-DMIDERHIVE_VERSION=1.0.0` 截断成 `...=1`，
...
| 项 | 取值 | 理由 |
|---|---|---|
| 安装范围 | **per-user**（`%LOCALAPPDATA%\MiderHive`） | 与既有 `scripts/deploy.ps1` 的约定一致；安装不需要管理员权限、不弹 UAC。企业批量下发可用 `-PerMachine` 切到 Program Files |
| 快捷方式 | 开始菜单 + 桌面 | 工作台是常驻托盘的应用，桌面入口更顺手 |
| 升级 | 同 `UpgradeCode` + `MajorUpgrade` | 新版覆盖安装，旧版先移除，避免文件残留 |
| 卸载 | 删除程序与快捷方式，**不动用户数据** | 用户数据在 `%USERPROFILE%\.miderhive`，不在安装包管理范围内；已实测卸载后数据完好 |
...
# 2) 装一次，确认程序能起、版本号正确
msiexec /i release\MiderHive-1.0.0-x64.msi
#    打开工作台 → 关于/侧栏应显示 v1.0.0；/api/health 也应返回 1.0.0

# 3) 卸载，确认用户数据未被删除
msiexec /x release\MiderHive-1.0.0-x64.msi
```
